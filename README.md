...
- 程序在危险模式下，严禁使用物理主机运行，在普通模式的基础上，乱点鼠标，乱按键盘，**！不要尝试结束进程！**因为程序使用 `NtSetInformationProcess`将自身设为关键进程，强行结束直接蓝屏
- 危险模式有可能会产生永久性的破坏，如果你开着文档、聊天、浏览器，会自动乱输入、误删内容，仅能在虚拟机内运行
- 危险模式会**持续修改UTC时间(人话：系统时间)**，软件授权失效、文件时间戳错乱、网络证书异常、部分游戏 / 办公软件报错
- 危险模式创建回收站肉眼名称完全一致，普通方式很难批量删除，大量占用桌面，清理步骤见后面内容。
...
## 解救方法
### 1. 运行安全模式
- 直接按下`Ctrl+Alt+Del`选择注销(新版系统是"退出登录")，若要保住文件，就等程序跑完自动退出
- 程序运行完成后自动退出，无需担心死循环驻留(危险模式、程序卡死除外)
### 2.普通模式
- 与安全模式一样，但是建议趁早退出
### **危险模式**
- 危险模式不一样，**⚠千万不要直接退出⚠**，程序无论如何，只要设置了系统关键进程，最后都是蓝屏
- 蓝屏过后，正常进入系统，系统非常的卡，因为系统时间与现实误差过大，系统接受不了
- 开启系统自动设置时间功能，等待自恢复
- 把多余的回收站删掉
