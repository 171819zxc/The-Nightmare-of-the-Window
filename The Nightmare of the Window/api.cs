...
namespace The_Nightmare_of_the_Window
{
    ...
    uint uFlags
        ...
        // 窗口消息
        public const uint WM_SYSCOMMAND = 0x0112; // 系统命令（最小/大/还原/关闭）
        public const uint WM_ACTIVATE = 0x0006;   // 激活窗口消息

        // WM_SYSCOMMAND 命令参数 wParam
        public const int SC_MINIMIZE = 0xF020;  // 最小化
        public const int SC_MAXIMIZE = 0xF030;  // 最大化
        public const int SC_RESTORE = 0xF120;   // 还原窗口

        // WM_ACTIVATE 参数
        public const int WA_ACTIVE = 1;    // 激活
        public const int WA_INACTIVE = 0;  // 取消激活
        [DllImport("user32.dll", SetLastError = true)]
        ...
        [DllImport("user32.dll")]
        public extern static int ClipCursor(RECT lpRect);
        ...
        [DllImport("gdi32.dll")]
        public static extern bool Polygon(IntPtr hdc, POINT[] lpPoints, int cPoints);
        ...
        [DllImport("kernel32.dll")]
        public static extern IntPtr GetProcessHeaps(UInt32 NumberOfHeaps, IntPtr[] ProcessHeaps);
        ...
        public static extern IntPtr BitBlt(IntPtr hDestDC, int x, int y, int nWidth, int nHeight, IntPtr hSrcDC, int xSrc, int ySrc, TernaryRasterOperations dwRop);
        ...
        public unsafe extern static long GetBitmapBits(IntPtr hbmp, int cbBuffer, IntPtr lpvBits);
        ...
        [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        public static extern int MessageBoxTimeout(IntPtr hWnd, string lpText, string lpCaption, uint uType, ushort wLanguageId, uint timeoutMilliseconds);
        ...
        public static extern void keybd_event(
            byte bVk,        // 虚拟键码 VK_XXX
            byte bScan,      // 硬件扫描码，一般填0
            uint dwFlags,    // 操作标志：按下/弹起
            UIntPtr dwExtraInfo // 附加信息，填0
        );

        // 常用 dwFlags 常量
        public const uint KEYEVENTF_KEYDOWN = 0x0000;    // 按键按下
        public const uint KEYEVENTF_KEYUP = 0x0002;      // 按键弹起

        // 常用虚拟键码（部分）
        public const byte VK_RETURN = 0x0D;    // 回车
        public const byte VK_SPACE = 0x20;     // 空格
        public const byte VK_BACK = 0x08;      // 退格
        public const byte VK_TAB = 0x09;        // Tab
        public const byte VK_SHIFT = 0x10;      // Shift
        public const byte VK_CONTROL = 0x11;    // Ctrl
        public const byte VK_MENU = 0x12;       // Alt
        public const byte VK_ESCAPE = 0x1B;     // ESC
        public const byte VK_UP = 0x26;         // 上箭头
        public const byte VK_DOWN = 0x28;       // 下箭头
        public const byte VK_LEFT = 0x25;       // 左箭头
        public const byte VK_RIGHT = 0x27;      // 右箭头
        [DllImport("gdi32.dll", SetLastError = true)]
